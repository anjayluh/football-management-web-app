...
function main() {
    ...
    if ($cmd === 'add') {
        ...
        $s .= $display;
        $title = ($id ? "Region $id" : 'Create Table');
    }
    if ($id) {
        if (isset($_GET['txt'])) {
...
function input_form($id) {
    ...
    <h1 align="center" background-color="red">Results from previous games</h1>
    <?php
...
function table($id) {
    ...
    $s = "<table><thead><tr><th>Team</th><th>Played</th><th>Won</th><th>GD</th><th>Lost</th><th>Avg</th><th>Pts</th></tr></thead><tbody>";
    foreach ($table as $team) {
        extract(hmap($team));
        $s .= "<tr><td>$name</td><td class=\"n\">$played</td><td class=\"n\">$won</td><td class=\"n\">$drawn</td><td class=\"n\">$lost</td><td class=\"n\">$against</td><td class=\"n\">$points</td></tr>";
    }
    $s .= "</tbody></table>";
...
function table_links() {
    ...
    foreach ($rs as $r) {
        extract(hmap($r));
        $s .= '<p><a href="?id=' . $id . '">Central Region ' . $id . '</a></p>';
    }
    return $s;
}
...
function head($title, $id) {
    ...
    <html>
        ...
        <body background-clor = "ghostwhite">

            <div class="container">
                <div class="span-10 last">
                    <h1 class = "hex">Table Generator</h1>
                    
                    <?php if ($title) { echo "<h2>$title</h2>"; } ?>
                    <?php if ($id) { ?>
                        <p><a href="<?php echo h($_SERVER['SCRIPT_NAME']); ?>">New Table</a></p>
                        <h3><a href="fixtures.php">Click here to generate match fixtures</a></h3>
                    <?php } ?>
